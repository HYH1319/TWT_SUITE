...
def solveTWTNOLINE_HARMONIC(
    C, b, d, beam_params, m, A0, y_end, N_harmonic, N_steps=1000
):
    ...
    def ode_system(y, state):
        ...
        # 计算积分项
        denominator = 1 + 2 * C * u
        sum_cos = np.sum(np.cos(N_harmonic*phi) / denominator) * delta_phi0
        sum_sin = np.sum(np.sin(N_harmonic*phi) / denominator) * delta_phi0
        ...
        term_u = (
            A * (1 - C * dtheta_dy) * np.sin(N_harmonic*phi)
            - C * dA_dy * np.cos(N_harmonic*phi)
            + Ez
        )
