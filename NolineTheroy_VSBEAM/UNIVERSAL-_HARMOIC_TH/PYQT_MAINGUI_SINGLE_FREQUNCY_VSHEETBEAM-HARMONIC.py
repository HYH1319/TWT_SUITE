...
from _TWT_CORE_NOLINE_COMPLEX_VU_VF import solveTWTNOLINE_OUTPUT, solveTWTNOLINE_HARMONIC, solveTWTNOLINE_Drift
...
class TWT_GUI:
    def __init__(self, root):
        ...
        self.default_segments = [
            {"len": 20, "Vpc": 0.292, "p_SWS": 0.50, "Kc": 1.0,  "Loss_perunit": 0, "Fn_K": 2, "type": "initial"},
            {"len": 6, "Vpc": 0.292, "p_SWS": 0.50, "Kc": 1.0,  "Loss_perunit": 0, "Fn_K": 2, "type": "attenuator"},
            {"len": 28, "Vpc": 0.289, "p_SWS": 0.25, "Kc": 2.0,  "Loss_perunit": 0.1, "Fn_K": 1.2, "type": "O"},
            {"len": 28, "Vpc": 0.284376, "p_SWS": 0.245, "Kc": 1.92,  "Loss_perunit": 0.1, "Fn_K": 1.2, "type": "O"},
            {"len": 28, "Vpc": 0.279752, "p_SWS": 0.240, "Kc": 1.84,  "Loss_perunit": 0.1, "Fn_K": 1.2, "type": "O"},
        ]
    ...
    def create_input_controls(self):
        ...
        # 谐波次数harmonic_times - 新增全局参数
        ttk.Label(global_frame, text="输入段谐波次数 harmonic_times:").grid(row=2, column=4, padx=5, pady=2, sticky=tk.W)
        self.harmonic_times = tk.DoubleVar(value=2)  # 谐波次数 默认值2
        ttk.Entry(global_frame, textvariable=self.harmonic_times, width=10).grid(row=2, column=5, padx=5, pady=2)
    ...
    def load_defaults(self):
        ...
        num_segments_to_load = 5
        for i in range(num_segments_to_load):
            # 在每次循环中，i 分别是 0, 1, 2, 3, 4
            seg_data = self.default_segments[i]  # 获取第 i 个分段的数据
            entries = self.segment_entries[i]    # 获取第 i 行UI的所有变量
            
            # 将数据填充到第 i 行
            entries[0].set(seg_data["len"])
            entries[1].set(seg_data["Vpc"])
            entries[2].set(seg_data["p_SWS"])
            entries[3].set(seg_data["Kc"])
            entries[4].set(seg_data["Loss_perunit"])
            entries[5].set(seg_data["Fn_K"])
            entries[6].set(seg_data["type"])
    ...
    def calculate(self):
        ...
        try:
            ...
            COMMON_PARAMS = {
                ...
                "harmonic_times":self.harmonic_times.get()
            }
            ...
            for seg_idx, seg in enumerate(SEGMENTS):
                ...
                if (COMMON_PARAMS["w"] == COMMON_PARAMS["t"]):  # 行波管等离子体频率降低系数R^2,ShitBeam用Fn,CicularBeam用Rowe特征值R^2
    ...
    def handle_initial_segment(self, params, common_params, calc_result, results):
        ...
        params.update({
            ...
            "N_harmonic":common_params["harmonic_times"],
        })
        self.result_text.insert(tk.END, f"\n初始段参数:\n{params}\n")
        self.result_text.insert(tk.END, f"使用输入功率: {P_in} W\n")  # 添加日志
        results.append(solveTWTNOLINE_HARMONIC(**params))
        self.result_text.update()
