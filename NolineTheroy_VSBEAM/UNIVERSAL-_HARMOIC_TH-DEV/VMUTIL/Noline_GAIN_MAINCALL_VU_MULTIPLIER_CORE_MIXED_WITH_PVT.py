...
from _TWT_CORE_NOLINE_COMPLEX_VU__HARMONIC_VF import (
...
def calculate_SEGMENT_TWT_NOLINE_SHEETBEAM(
    ...
    def handle_harmonic_parameters(param_list, harmonic_index, segment_count):
        ...
        if (
            ...
            and len(param_list) == 2
            and harmonic_index is not None
        ):
            if harmonic_index < 0 or harmonic_index >= segment_count:
                raise ValueError(
                    f"倍频起始索引 {harmonic_index} 必须在 0 到 {segment_count-1} 之间"
                )
            # 返回完整列表：前段值重复harmonic_index次 + 倍频段值重复剩余次
            return [param_list[0]] * harmonic_index + [param_list[1]] * (
                segment_count - harmonic_index
            )
        return param_list

    def build_segment_input(common_params, seg,f0_GHz_current):
        """构建单个段的计算输入参数"""
        return [
            common_params["I"],
            common_params["V"],
    ...
    def initialize_first_segment(params, common, results, input_power):
        """初始化并计算第一段"""
    ...
    if harmonic_start_idx is not None:
        ...
        Loss_perunit = handle_harmonic_parameters(
            Loss_perunit, harmonic_start_idx, num_segments
        )
        ...
        harmonic_times = handle_harmonic_parameters(
            harmonic_times, harmonic_start_idx, num_segments
        )
        ...
        segments.append(
            {
                ...
                "harmonic_times": expansion_params["harmonic_times"][i],
            }
        )
    ...
    for seg_idx, segment in enumerate(segments):
        # 4.1 准备当前段输入参数
        # 谐波倍频矫正
        f0_GHz_current=common_params["f0_GHz"]/segment["harmonic_times"]
        input_params = build_segment_input(common_params, segment,f0_GHz_current)
        ...
        solver_params = {
            ...
            "m": 50,  # 求解器精度参数
            "y_end": 2 * np.pi * C*(calc_result["互作用长度N"]) + (results[-1]["y"][-1] if seg_idx > 0 else 0),
        }
        print(f"段 {seg_idx+1} 求解参数:{solver_params}\n")
        # 4.4 根据段类型处理
        if seg_idx == 0:  # 第一段
            initialize_first_segment(
                solver_params, common_params, results, P_in
            )
...
if __name__ == "__main__":
    ...
    input_params = [
        0.30,  # I [A],公共参数
        23000,  # V [V],公共参数
        [1.0, 2.0],  # 耦合系数 (Kc)，倍频前和倍频段两组值
        [0, 0.1],  # 单位损耗，倍频前和倍频段两组值
        [1],  # 衰减段索引列表
        [0.50, 0.50, 0.25, 0.245, 0.240],  # 慢波结构周期
        [20, 6, 28, 28, 28],  # 各段长度（周期数）
        0.40,  # 带状注宽度 w [mm],公共参数
        0.10,  # 带状注厚度 t [mm],公共参数
        [2, 1.2],  # 填充因子，倍频前和倍频段两组值
        440,  # 中心频率 f0 [GHz],公共参数
        [0.292, 0.289],  # 相位速度，倍频前和倍频段两组值
        0.005,  # 输入功率 [W],公共参数
        30,  # 衰减器损耗 [dB],公共参数
        [2, 1],  # 输入端谐波次数harmonic_times,公共参数
    ]
    ...
    # 自定义调整系数
    Vpc_adjust_coef = 0.80  # Vpc调整系数
    Kc_adjust_coef = 2.0  # Kc调整系数
    ...
    for i, seg in enumerate(result["各段参数"]):
        ...
        print(
            f"段 {i+1} ({harmonic_type}): Kc={seg['Kc']:.4f} Ω, Vpc={seg['Vpc']:.4f}c"
        )
