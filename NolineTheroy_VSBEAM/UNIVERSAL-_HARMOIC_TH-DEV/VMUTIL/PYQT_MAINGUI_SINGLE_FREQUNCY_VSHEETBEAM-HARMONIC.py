...
from _TWT_CORE_NOLINE_COMPLEX_VU__HARMONIC_VF import solveTWTNOLINE_OUTPUT, solveTWTNOLINE_INIT, solveTWTNOLINE_Drift
...
class TWT_GUI:
    ...
    def __init__(self, root):
        ...
        self.default_segments = [
            {"len": 20, "Vpc": 0.292, "p_SWS": 0.50, "Kc": 1.0,  "Loss_perunit": 0, "Fn_K": 2, "harmonic_times": 2, "type": "initial"},
            ...
            {"len": 28, "Vpc": 0.284376, "p_SWS": 0.245, "Kc": 1.92,  "Loss_perunit": 0.1, "Fn_K": 1.2, "harmonic_times": 1, "type": "O"},
            {"len": 28, "Vpc": 0.279752, "p_SWS": 0.240, "Kc": 1.84,  "Loss_perunit": 0.1, "Fn_K": 1.2, "harmonic_times": 1, "type": "O"},
        ]
    ...
    def create_input_controls(self):
        ...
        self.P_in_var = tk.DoubleVar(value=0.005)  # 默认值0.004W
        ...
        self.f0_GHz_var = tk.DoubleVar(value=440)  # 默认值440 GHz
    ...
    def load_defaults(self):
        ...
        self.P_in_var.set(0.005)  # P_in默认值
        self.w_var.set(0.4)
        self.t_var.set(0.1)
    ...
    def calculate(self):
        ...
        try:
            ...
            for seg_idx, seg in enumerate(SEGMENTS):
                ...
                # 参数计算
                # 谐波倍频矫正
                f0_GHz_current=COMMON_PARAMS["f0_GHz"]/seg["harmonic_times"]
                input_params = self.build_input_params(COMMON_PARAMS, seg,f0_GHz_current)
                ...
                # 互作用长度L
                L=2 * np.pi * C*(calc_result["互作用长度N"])

                # beam_params初始化-束流尺寸与填充
                beam_params = []
                #Sheet Beam Rowe 特征值
                Rn_sqr_Vaules = calc_result["等离子体频率降低因子Fn"]**2
                Wp_w = calc_result["等离子体频率Wp"] / (2 * np.pi * (f0_GHz_current) * 1e9)
                Space_cut = 10
                beam_params = [Rn_sqr_Vaules, Wp_w, Space_cut]
                ...
                # 分段处理
                if seg["type"] == "initial":
                    self.handle_initial_segment(params, COMMON_PARAMS, results)
    ...
    def build_input_params(self, common_params, seg,f0_GHz_current):
        """构建输入参数列表"""
        return [
            common_params["I"], 
            common_params["V"], 
    ...
    def handle_initial_segment(self, params, common_params, results):
        """处理初始段"""
        # 使用全局参数中的输入功率P_in
