...
from _TWT_CORE_NOLINE_COMPLEX_VSHEETBEAM_VFTSSM import solveTWTNOLINE_OUTPUT, solveTWTNOLINE_INIT, solveTWTNOLINE_Drift
...
class TWT_GUI:
    ...
    def process_and_visualize(self, results, C_list, common_params, segments, Loss_attu):
        ...
        self.visualize_results(Y_Finall, A_Fianll, theta_Fianll, u_now, phi_now, Lenth, P_Out, common_params['P_in'], results[-1])

    def visualize_results(self, Y, A, theta, u,phi, Lenth, P_Out, P_in, final_seg):
        ...
        ax4.plot(Y, phi, 'navy'); ax4.set_xlabel("Position y"); ax4.set_ylabel("Electron Phase"); ax4.set_title("Electron Phase Space"); ax4.grid(alpha=0.3)
        fig_phase_dist = plt.figure(figsize=(8, 6)); ax_phase_dist = fig_phase_dist.add_subplot(111)
        ax_phase_dist.plot(Lenth, u, 'navy'); ax_phase_dist.set_xlabel("Position Z"); ax_phase_dist.set_ylabel("Electron Phase"); ax_phase_dist.set_title("Electron Phase Space"); ax_phase_dist.grid(alpha=0.3)
        ...
        ax5.plot(Lenth, u, 'navy'); ax5.set_xlabel("Position Z"); ax5.set_ylabel("Electron Velocity"); ax5.set_title("Electron Velocity Space"); ax5.grid(alpha=0.3)
        fig_elec = plt.figure(figsize=(8, 6)); ax_elec = fig_elec.add_subplot(111)
        ax_elec.plot(Lenth, u, 'navy'); ax_elec.set_xlabel("Position Z"); ax_elec.set_ylabel("Electron Velocity"); ax_elec.set_title("Electron Velocity Space"); ax_elec.grid(alpha=0.3)
