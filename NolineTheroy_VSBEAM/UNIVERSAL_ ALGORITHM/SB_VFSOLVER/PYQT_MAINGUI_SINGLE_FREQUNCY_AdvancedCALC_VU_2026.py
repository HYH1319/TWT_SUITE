...
from _TWT_CORE_NOLINE_COMPLEX_VSHEETBEAM_VFTSSM import (
...
class TWTCalculator:
    ...
    def calculate(common_params, segments, loss_attu, log_callback=None):
        ...
        for seg_idx, seg in enumerate(segments):
            ...
            Rn_sqr_values = (
                calc_result["Rowe特征值R"] ** 2
                if common_params["w"] == common_params["t"]
                else calc_result["等离子体频率降低因子Fn"] ** 2
            )
            wp_omega = calc_result["等离子体频率Wp"] / (
                2 * np.pi * common_params["f0"] * 1e9
            )  # 使用全局频率
            Space_CUT = 20
