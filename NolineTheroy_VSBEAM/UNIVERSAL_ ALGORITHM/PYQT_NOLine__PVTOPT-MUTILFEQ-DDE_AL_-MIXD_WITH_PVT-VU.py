...
class DistributedDETask(QThread):
    ...
    def run(self):
        try:
            ...
            for iter_num in range(1, self.max_iter + 1):
                ...
                if iter_num % 5 == 0 or iter_num == self.max_iter:
    ...
    def _mutate_vectorized(self, pop, fitness):
        ...
        if self.strategy == 'best1bin':
            ...
            idx1 = np.random.randint(0, n_pop, n_pop)
            idx2 = np.random.randint(0, n_pop, n_pop)
            
            mutant = donor + self.F * (pop[idx1] - pop[idx2])
        
        elif self.strategy == 'rand1bin':
            idx0 = np.random.randint(0, n_pop, n_pop)
            idx1 = np.random.randint(0, n_pop, n_pop)
            idx2 = np.random.randint(0, n_pop, n_pop)
            
            mutant = pop[idx0] + self.F * (pop[idx1] - pop[idx2])
            ...
            mutant = pop + self.F * (donor - pop + pop[idx1] - pop[idx2])
        ...
        mutant = np.clip(mutant, self.bounds_low, self.bounds_high)
