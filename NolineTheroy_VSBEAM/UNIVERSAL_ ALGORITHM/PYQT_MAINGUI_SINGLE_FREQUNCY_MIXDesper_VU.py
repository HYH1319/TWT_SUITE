...
from _TWT_CORE_NOLINE_COMPLEX_VSHEETBEAM import solveTWTNOLINE_OUTPUT, solveTWTNOLINE_INIT, solveTWTNOLINE_Drift
...
class TWT_GUI:
    ...
    def load_defaults(self):
        ...
        self.f0_GHz_var.set(211)  # 设置频率默认值
    ...
    def calculate(self):
        ...
        try:
            ...
            for seg_idx, seg in enumerate(SEGMENTS):
                ...
                wp_omega = calc_result["等离子体频率Wp"] / (2 * np.pi * COMMON_PARAMS["f0_GHz"] * 1e9)  # 使用全局频率
                Space_CUT=10
    ...
    def process_and_visualize(self, results, C_list, common_params, segments, Loss_attu):
        ...
        self.result_text.insert(tk.END, f"Loss_attu: {Loss_attu}\n")
        self.result_text.insert(tk.END, f"频率: {common_params['f0_GHz']} GHz\n")  # 显示频率
        self.result_text.see(tk.END)
        self.result_text.config(state=tk.DISABLED)
