...
class FixedParamsEditor(BaseEditor):
    ...
    def __init__(self, parent=None):
        fields = [
            ("电子枪参数", "label"),
            ("电流 I (A):", "double", 0, 5, 0.07, 0.001, 4),
            ("电压 V (V):", "double", 0, 30000, 23000, 100, 0),
            
            ("慢波结构参数", "label"),
            ("倍频前Kc值 (Ω):", "double", 0, 10, 0.363, 0.001, 4),
            ("倍频段Kc值 (Ω):", "double", 0, 10, 1.82, 0.001, 4),
            ("倍频前每单元损耗:", "double", 0, 1e3, 0.1, 0.001, 4),
            ("倍频段每单元损耗:", "double", 0, 1e3, 0.25, 0.001, 4),
            ("衰减段索引 (逗号分隔):", "text", "1"),
            ("各段周期数 (逗号分隔):", "text", "30,5,5,5,5"),
            
            ("束流参数", "label"),
            ("束流宽度 w (mm):", "double", 0, 10, 0.04, 0.001, 4),
            ("束流厚度 t (mm):", "double", 0, 10, 0.04, 0.001, 4),
            
            ("填充参数", "label"),
            ("倍频前填充因子:", "double", 1, 10, 2.0, 0.001, 4),
            ("倍频段填充因子:", "double", 1, 10, 1.1, 0.001, 4),
            
            ("频率参数", "label"),
            ("工作频率 (GHz):", "double", 0, 3000, 1354.0, 1, 1),
            
            ("输入/衰减参数", "label"),
            ("输入功率 (W):", "double", 0, 100, 0.0002, 0.0001, 5),
            ("衰减量 (dB):", "double", 0, 100, 20, 1, 1),
            
            ("相位参数", "label"),
            ("倍频前Vpc值 (c):", "double", 0, 1, 0.2887, 0.0001, 5),
            ("倍频段Vpc值 (c):", "double", 0, 1, 0.2902, 0.0001, 5),
    ...
    def get_params(self):
        ...
        try:
            ...
            # 两组色散参数
            params["kc"] = [
                self.get_value("倍频前Kc值 (Ω):"),
                self.get_value("倍频段Kc值 (Ω):")
            ]
            params["loss_perunit"] = [
                self.get_value("倍频前每单元损耗:"),
                self.get_value("倍频段每单元损耗:")
            ]
            params["Fn_K"] = [
                self.get_value("倍频前填充因子:"),
                self.get_value("倍频段填充因子:")
            ]
            params["vpc"] = [
                self.get_value("倍频前Vpc值 (c):"),
                self.get_value("倍频段Vpc值 (c):")
            ]
            
            # 倍频起始索引处理
            harmonic_idx_str = self.get_value("倍频起始索引 (0开始):")
            if harmonic_idx_str == "":
...
class DDEConfigEditor(BaseEditor):
    ...
    def __init__(self, parent=None):
        fields = [
            ...
            ("p_SWS下限 (逗号分隔):", "text", "0.320,0.315,0.315,0.315,0.320,0.0746,0.074,0.074,0.074"),
            ("p_SWS上限 (逗号分隔):", "text", "0.320,0.325,0.325,0.315,0.320,0.0746,0.075,0.075,0.075"),
...
class InitialParamsEditor(BaseEditor):
    ...
    def __init__(self, parent=None):
        fields = [
            ("初始p_SWS值 (逗号分隔):", "text", "0.320,0.321,0.320,0.319,0.320,0.0746,0.075,0.0746,0.0742"),
        ]
        super().__init__(fields, parent)
...
class TWTMainWindow(QMainWindow):
    ...
    def start_optimization(self):
        ...
        self.log_area.append(f"Vpc调整系数: {fixed_params['vpc_coeff']}, Kc调整系数: {fixed_params['kc_coeff']}")
