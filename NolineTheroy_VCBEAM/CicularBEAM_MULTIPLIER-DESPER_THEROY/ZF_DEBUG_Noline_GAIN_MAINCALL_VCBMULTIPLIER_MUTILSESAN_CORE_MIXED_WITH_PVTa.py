...
if __name__ == "__main__":
    ...
    input_params = [
        0.30,     # I [A],公共参数
        23000,    # V [V],公共参数
        [3.6, 3.2, 3.0,2.8],  # 耦合系数 (Kc)，倍频前、倍频A和倍频B_X段值
        [0, 0.05, 0.1,0.15],   # 单位损耗，倍频前、倍频A和倍频B_X段值
        [0],      # 衰减段索引列表
        [0.50,0.25, 0.245, 0.200, 0.195, 0.150],  # 慢波结构周期
        [15,15, 5, 5, 5, 5],  # 各段长度（周期数）
        0.20,     # 带状注宽度 w [mm],公共参数
        0.20,     # 带状注厚度 t [mm],公共参数
        [2, 1.5, 1.2,1.1], # 填充因子，倍频前、倍频A和倍频B_X段值
        211,      # 中心频率 f0 [GHz],公共参数
        [0.290, 0.288, 0.287,0.286],  # 相位速度，倍频前、倍频A和倍频B_X段值
        0.1,      # 输入功率 [W],公共参数
        15,       # 衰减器损耗 [dB],公共参数
    ]

    # 倍频段划分 [倍频A起始索引, 倍频B起始索引]
    harmonic_sections = [1,3,5]  # 段0:倍频前, 段1-2:倍频A, 段3-4:倍频B
