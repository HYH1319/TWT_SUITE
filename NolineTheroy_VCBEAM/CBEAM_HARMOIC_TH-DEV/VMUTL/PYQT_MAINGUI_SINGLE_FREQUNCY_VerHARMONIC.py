...
from _TWT_CORE_NOLINE_COMPLEX_VCBEAM_HARMONIC_VF import solveTWTNOLINE_OUTPUT, solveTWTNOLINE_INIT, solveTWTNOLINE_Drift

class TWT_GUI:
    # 定义类常量，统一管理参数列
    SEGMENT_COLUMNS = ("len", "Vpc", "p_SWS", "Kc", "Loss_perunit", "Fn_K", "harmonic_times", "type")
    SEGMENT_HEADERS = ("周期数(个)", "Vpc", "螺距(mm)", "耦合阻抗", "每单元损耗", "填充因子", "谐波次数", "类型")
    ...
    def __init__(self, root):
        ...
        self.default_segments = [
            {"len": 10, "Vpc": 0.292, "p_SWS": 0.50, "Kc": 1.0,  "Loss_perunit": 0, "Fn_K": 2, "harmonic_times": 2, "type": "initial"},
            {"len": 10, "Vpc": 0.292, "p_SWS": 0.50, "Kc": 1.0,  "Loss_perunit": 0, "Fn_K": 2, "harmonic_times": 2, "type": "Harmonic_Segment"},
            {"len": 6, "Vpc": 0.292, "p_SWS": 0.50, "Kc": 1.0,  "Loss_perunit": 0, "Fn_K": 2, "harmonic_times": 2, "type": "attenuator"},
            {"len": 28, "Vpc": 0.289, "p_SWS": 0.25, "Kc": 2.0,  "Loss_perunit": 0.1, "Fn_K": 1.2, "harmonic_times": 1, "type": "O"},
            {"len": 28, "Vpc": 0.2844, "p_SWS": 0.245, "Kc": 1.92,  "Loss_perunit": 0.1, "Fn_K": 1.2, "harmonic_times": 1, "type": "O"},
            {"len": 28, "Vpc": 0.2798, "p_SWS": 0.240, "Kc": 1.84,  "Loss_perunit": 0.1, "Fn_K": 1.2, "harmonic_times": 1, "type": "O"},
        ]
    ...
    def create_input_controls(self):
        ...
        self.P_in_var = tk.DoubleVar(value=0.005)
        ...
        self.loss_attu_var = tk.DoubleVar(value=30)
        ...
        self.f0_GHz_var = tk.DoubleVar(value=440)
        ...
        # 使用类常量创建表头
        for col, header in enumerate(self.SEGMENT_HEADERS):
        ...
        for i in range(10):  # 最多10个分段
            ...
            # 使用类常量创建输入框
            for j, param in enumerate(self.SEGMENT_COLUMNS):
                var = tk.StringVar()
        ...
        # 类型选择列添加下拉菜单（修正索引为7）
        type_options = ["initial", "Harmonic_Segment","attenuator", "O"]
        for i, entry_frame in enumerate(entry_frames):
            var = self.segment_entries[i][7]  # 修正：第8个元素（索引7）
            combo = ttk.Combobox(entry_frame, textvariable=var, values=type_options, width=8)
            combo.grid(row=0, column=7, padx=2, pady=2)  # 修正：第8列
    ...
    def load_defaults(self):
        ...
        self.P_in_var.set(0.005)
        self.w_var.set(0.2)
        self.t_var.set(0.2)
        self.loss_attu_var.set(30)
        self.f0_GHz_var.set(440)

        # 加载分段参数
        num_segments_to_load = min(len(self.default_segments), len(self.segment_entries))
        for i in range(num_segments_to_load):
            seg_data = self.default_segments[i]
            entries = self.segment_entries[i]
            
            # 检查entries长度是否足够
            if len(entries) < len(self.SEGMENT_COLUMNS):
                raise ValueError(f"参数列配置错误，预期{len(self.SEGMENT_COLUMNS)}个参数，实际{len(entries)}个")
            
            # 使用类常量填充数据
            for j, param in enumerate(self.SEGMENT_COLUMNS):
                entries[j].set(seg_data[param])
        ...
        try:
            ...
            for row in self.segment_entries:
                ...
                if any(entry.get() for entry in row):
                    seg = {}
                    for i, param in enumerate(self.SEGMENT_COLUMNS):
                        # 尝试转换为数值，否则保留为字符串
                        value = row[i].get()
                        try:
                            value = float(value)
            ...
            for seg_idx, seg in enumerate(SEGMENTS):
                ...
                # 参数计算
                # 谐波矫正
                f0_GHz_cuurent=COMMON_PARAMS["f0_GHz"] / seg["harmonic_times"]
                input_params = self.build_input_params(COMMON_PARAMS, seg,f0_GHz_cuurent)
                ...
                params = {
                    ...
                    "m": 50,  
                    #频率强相关参数
                    "wp_w": calc_result["等离子体频率Wp"] / (2 * np.pi * f0_GHz_cuurent * 1e9),
                    "beta_space": calc_result["beta_Space"],
                    "y_end": 2 * np.pi *C* calc_result["互作用长度N"] + (results[-1]["y"][-1] if seg_idx > 0 and results else 0)
                }
                # 在文本区域显示求解器参数
                self.result_text.insert(tk.END, f"求解器参数:\n")
                ...
                # 分段处理
                if seg["type"] == "initial":# 初始段
                    self.initialize_harmonicfirst_segment(params, COMMON_PARAMS, results)
                elif seg["type"] == "Harmonic_Segment":  # 倍频前段
                    self.process_harmonicnormal_segment(params, results, seg_idx)
                elif seg["type"] == "attenuator":# 衰减段
                    self.handle_attenuator_segment(params, results, seg_idx, Loss_attu)
                else:
    ...
    def build_input_params(self, common_params, seg,f0_GHz_cuurent):
        """构建输入参数列表"""
        return [
            common_params["I"], 
            common_params["V"], 
    ...
    def initialize_harmonicfirst_segment(self, params, common_params, results):
        """初始化并计算第一段"""
    ...
    def process_harmonicnormal_segment(self,params, results, seg_idx):
        """处理倍频前输出段"""
    ...
    def handle_attenuator_segment(self, params, results, seg_idx, Loss_attu):
        """处理衰减段"""
        prev = results[seg_idx-1]
        params.update({
            "result_y_ends": prev["y"][-1],
