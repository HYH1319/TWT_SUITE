...
# 替换为您的实际计算函数
from Noline_GAIN_MAINCALL_VCBMULTIPLIER_CORE_MIXED_WITH_PVT import calculate_SEGMENT_TWT_NOLINE
...
# 配置文件名
CONFIG_FILE = "./configUDF/OPT/twt_DDE_config_3freq.json"
    ...
    try:
        ...
        for freq_idx in range(3):
            result = calculate_SEGMENT_TWT_NOLINE(
                ...
                Kc=_global_fixed_params[f"kc_freq{freq_idx+1}"],
                Loss_perunit=_global_fixed_params[f"loss_perunit_freq{freq_idx+1}"],
                ...
                Fn_K=_global_fixed_params[f"Fn_K_freq{freq_idx+1}"],
                f0_GHz=_global_fixed_params[f"f0_GHz_freq{freq_idx+1}"],
                Vpc=_global_fixed_params[f"vpc_freq{freq_idx+1}"],
...
class OptimizedDistributedDETask(QThread):
    ...
    initial_power_calculated = pyqtSignal(float, float, float, float)
    ...
    def run(self):
        try:
            ...
            if self.initial_p is not None and len(self.initial_p) == self.n_dim:
                ...
                for freq_idx in range(3):
                    result = calculate_SEGMENT_TWT_NOLINE(
                        ...
                        Kc=self.fixed_params[f"kc_freq{freq_idx+1}"],
                        Loss_perunit=self.fixed_params[f"loss_perunit_freq{freq_idx+1}"],
                        SectionedSEGMENT_IDX=self.fixed_params["section_seg_idx"],
                        p_SWS=self.initial_p,
                        N_unit=self.fixed_params["n_unit"],
                        w=self.fixed_params["w"],
                        t=self.fixed_params["t"],
                        Fn_K=self.fixed_params[f"Fn_K_freq{freq_idx+1}"], 
                        f0_GHz=self.fixed_params[f"f0_GHz_freq{freq_idx+1}"],
                        Vpc=self.fixed_params[f"vpc_freq{freq_idx+1}"],
                        P_in=self.fixed_params["p_in"],
                        Loss_attu=self.fixed_params["loss_attu"],
                        harmonic_times=self.fixed_params["harmonic_times"],
                        harmonic_start_idx=self.fixed_params["harmonic_start_idx"],
                        Vpc_adjust_coef=self.fixed_params["vpc_coeff"],
                        Kc_adjust_coef=self.fixed_params["kc_coeff"]
                    )
                    powers.append(result["输出功率P_out"])
                
                self.initial_power_calculated.emit(powers[0], powers[1], powers[2], total_power)
            ...
            self.log_message.emit(f"初始最佳总功率: {best_power:.2f} W")
            ...
            if self.is_running:
                self.finished.emit({
                    ...
                    'best_total_power': -self.best_fitness,
...
class ThreeFreqHarmonicEditor(BaseEditor):
    """三频点倍频参数编辑器"""
    def __init__(self, parent=None):
        fields = [
            ("━━━ 倍频起始索引 ━━━", "label"),
            ...
            ("━━━ 频点1参数 ━━━", "label"),
            ("频点1工作频率 (GHz):", "double", 0, 3000, 440.0, 1, 1),
            ("频点1倍频前 Kc (Ω):", "double", 0, 10, 1.0, 0.001, 5),
            ("频点1倍频段 Kc (Ω):", "double", 0, 10, 2.0, 0.001, 5),
            ("频点1倍频前 Vpc (c):", "double", 0, 1, 0.292, 0.0001, 5),
            ("频点1倍频段 Vpc (c):", "double", 0, 1, 0.289, 0.0001, 5),
            ("频点1倍频前 填充因子:", "double", 1, 10, 2.0, 0.001, 5),
            ("频点1倍频段 填充因子:", "double", 1, 10, 1.2, 0.001, 5),
            ("频点1倍频前 单元损耗:", "double", 0, 1e3, 0.0, 0.001, 5),
            ("频点1倍频段 单元损耗:", "double", 0, 1e3, 0.1, 0.001, 5),
            
            ("━━━ 频点2参数 ━━━", "label"),
            ("频点2工作频率 (GHz):", "double", 0, 3000, 445.0, 1, 1),
            ("频点2倍频前 Kc (Ω):", "double", 0, 10, 1.0, 0.001, 5),
            ("频点2倍频段 Kc (Ω):", "double", 0, 10, 2.0, 0.001, 5),
            ("频点2倍频前 Vpc (c):", "double", 0, 1, 0.292, 0.0001, 5),
            ("频点2倍频段 Vpc (c):", "double", 0, 1, 0.289, 0.0001, 5),
            ("频点2倍频前 填充因子:", "double", 1, 10, 2.0, 0.001, 5),
            ("频点2倍频段 填充因子:", "double", 1, 10, 1.2, 0.001, 5),
            ("频点2倍频前 单元损耗:", "double", 0, 1e3, 0.0, 0.001, 5),
            ("频点2倍频段 单元损耗:", "double", 0, 1e3, 0.1, 0.001, 5),
            
            ("━━━ 频点3参数 ━━━", "label"),
            ("频点3工作频率 (GHz):", "double", 0, 3000, 450.0, 1, 1),
            ("频点3倍频前 Kc (Ω):", "double", 0, 10, 1.0, 0.001, 5),
            ("频点3倍频段 Kc (Ω):", "double", 0, 10, 2.0, 0.001, 5),
            ("频点3倍频前 Vpc (c):", "double", 0, 1, 0.292, 0.0001, 5),
            ("频点3倍频段 Vpc (c):", "double", 0, 1, 0.289, 0.0001, 5),
            ("频点3倍频前 填充因子:", "double", 1, 10, 2.0, 0.001, 5),
            ("频点3倍频段 填充因子:", "double", 1, 10, 1.2, 0.001, 5),
            ("频点3倍频前 单元损耗:", "double", 0, 1e3, 0.0, 0.001, 5),
            ("频点3倍频段 单元损耗:", "double", 0, 1e3, 0.1, 0.001, 5),
        ]
        super().__init__(fields, parent)
...
def collect_all_params(basic_editor, harmonic_editor):
    ...
    try:
        ...
        # 频点1参数
        params["f0_GHz_freq1"] = harmonic_editor.get_value("频点1工作频率 (GHz):")
        params["kc_freq1"] = [
            harmonic_editor.get_value("频点1倍频前 Kc (Ω):"),
            harmonic_editor.get_value("频点1倍频段 Kc (Ω):")
        ]
        params["vpc_freq1"] = [
            harmonic_editor.get_value("频点1倍频前 Vpc (c):"),
            harmonic_editor.get_value("频点1倍频段 Vpc (c):")
        ]
        params["Fn_K_freq1"] = [
            harmonic_editor.get_value("频点1倍频前 填充因子:"),
            harmonic_editor.get_value("频点1倍频段 填充因子:")
        ]
        params["loss_perunit_freq1"] = [
            harmonic_editor.get_value("频点1倍频前 单元损耗:"),
            harmonic_editor.get_value("频点1倍频段 单元损耗:")
        ]
        
        # 频点2参数
        params["f0_GHz_freq2"] = harmonic_editor.get_value("频点2工作频率 (GHz):")
        params["kc_freq2"] = [
            harmonic_editor.get_value("频点2倍频前 Kc (Ω):"),
            harmonic_editor.get_value("频点2倍频段 Kc (Ω):")
        ]
        params["vpc_freq2"] = [
            harmonic_editor.get_value("频点2倍频前 Vpc (c):"),
            harmonic_editor.get_value("频点2倍频段 Vpc (c):")
        ]
        params["Fn_K_freq2"] = [
            harmonic_editor.get_value("频点2倍频前 填充因子:"),
            harmonic_editor.get_value("频点2倍频段 填充因子:")
        ]
        params["loss_perunit_freq2"] = [
            harmonic_editor.get_value("频点2倍频前 单元损耗:"),
            harmonic_editor.get_value("频点2倍频段 单元损耗:")
        ]
        
        # 频点3参数
        params["f0_GHz_freq3"] = harmonic_editor.get_value("频点3工作频率 (GHz):")
        params["kc_freq3"] = [
            harmonic_editor.get_value("频点3倍频前 Kc (Ω):"),
            harmonic_editor.get_value("频点3倍频段 Kc (Ω):")
        ]
        params["vpc_freq3"] = [
            harmonic_editor.get_value("频点3倍频前 Vpc (c):"),
            harmonic_editor.get_value("频点3倍频段 Vpc (c):")
        ]
        params["Fn_K_freq3"] = [
            harmonic_editor.get_value("频点3倍频前 填充因子:"),
            harmonic_editor.get_value("频点3倍频段 填充因子:")
        ]
        params["loss_perunit_freq3"] = [
            harmonic_editor.get_value("频点3倍频前 单元损耗:"),
            harmonic_editor.get_value("频点3倍频段 单元损耗:")
        ]
...
class TWTMainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("三频点倍频行波管分布式差分进化优化工具 v3.0")
    ...
    def init_ui(self):
        ...
        # 状态显示
        status_group = QGroupBox("📊 三频点优化状态")
        status_layout = QGridLayout(status_group)
        
        self.status_labels = {
            "freq1_power": ("频点1功率", "#e3f2fd"),
            "freq2_power": ("频点2功率", "#fff3e0"),
            "freq3_power": ("频点3功率", "#f3e5f5"),
            "total_power": ("总功率", "#e8f5e9")
        }
    ...
    def start_optimization(self):
        ...
        try:
            ...
            for freq_idx in range(3):
                result = calculate_SEGMENT_TWT_NOLINE(
                    I=fixed_params["i"],
                    V=fixed_params["v"],
                    Kc=fixed_params[f"kc_freq{freq_idx+1}"],
                    Loss_perunit=fixed_params[f"loss_perunit_freq{freq_idx+1}"],
                    SectionedSEGMENT_IDX=fixed_params["section_seg_idx"],
                    p_SWS=initial_p,
                    N_unit=fixed_params["n_unit"],
                    w=fixed_params["w"],
                    t=fixed_params["t"],
                    Fn_K=fixed_params[f"Fn_K_freq{freq_idx+1}"],
                    f0_GHz=fixed_params[f"f0_GHz_freq{freq_idx+1}"],
                    Vpc=fixed_params[f"vpc_freq{freq_idx+1}"],
            ...
            # 显示初始功率
            self.handle_initial_power(powers[0], powers[1], powers[2], total_power)
            self.log_message(f"✓ 初始参数功率计算完成:")
            self.log_message(f"  频点1: {powers[0]:.2f} W")
            self.log_message(f"  频点2: {powers[1]:.2f} W")
            self.log_message(f"  频点3: {powers[2]:.2f} W")
            self.log_message(f"  总功率: {total_power:.2f} W")
        except Exception as e:
        ...
        self.log_area.append("  三频点倍频行波管分布式差分进化优化 - v3.0")
        self.log_area.append("=" * 60)
        self.log_area.append(f"📐 参数维度: {n_dim}")
        self.log_area.append(f"🎯 初始p_SWS: {initial_p}")
        self.log_area.append(f"⚡ 频点1 (f={fixed_params['f0_GHz_freq1']} GHz): {powers[0]:.2f} W")
        self.log_area.append(f"⚡ 频点2 (f={fixed_params['f0_GHz_freq2']} GHz): {powers[1]:.2f} W")
        self.log_area.append(f"⚡ 频点3 (f={fixed_params['f0_GHz_freq3']} GHz): {powers[2]:.2f} W")
        self.log_area.append(f"⚡ 初始总功率: {total_power:.2f} W")
    ...
    def handle_initial_power(self, power1, power2, power3, total):
        """处理初始功率信号"""
        self.status_values["freq1_power"].setText(f"{power1:.2f}")
        self.status_values["freq2_power"].setText(f"{power2:.2f}")
        self.status_values["freq3_power"].setText(f"{power3:.2f}")
        self.status_values["total_power"].setText(f"{total:.2f}")
    
    def handle_iteration_complete(self, iteration, total_power):
        """处理迭代完成信号"""
        self.iter_label.setText(f"🔄 当前迭代: {iteration}")
        self.status_values["total_power"].setText(f"{total_power:.2f}")
    
    def handle_progress_update(self, progress, best_total_power):
        """处理进度更新信号"""
        self.progress_bar.setValue(progress)
        self.status_values["total_power"].setText(f"{best_total_power:.2f}")
    ...
    def optimization_finished(self, result):
        ...
        try:
            powers = []
            for freq_idx in range(3):
                res = calculate_SEGMENT_TWT_NOLINE(
                    I=_global_fixed_params["i"],
                    V=_global_fixed_params["v"],
                    Kc=_global_fixed_params[f"kc_freq{freq_idx+1}"],
                    Loss_perunit=_global_fixed_params[f"loss_perunit_freq{freq_idx+1}"],
                ...
                powers.append(res["输出功率P_out"])
            
            # 更新显示
            self.status_values["freq1_power"].setText(f"{powers[0]:.2f}")
            self.status_values["freq2_power"].setText(f"{powers[1]:.2f}")
            self.status_values["freq3_power"].setText(f"{powers[2]:.2f}")
            self.status_values["total_power"].setText(f"{result['best_total_power']:.2f}")
            
            # 保存各频点功率到结果中
            result['freq1_power'] = powers[0]
            result['freq2_power'] = powers[1]
            result['freq3_power'] = powers[2]
        except Exception as e:
            self.log_message(f"计算各频点功率失败: {str(e)}")
            powers = [0, 0, 0]
        
        self.log_area.append("=" * 60)
        self.log_message(f"✓ 优化完成!")
        self.log_message(f"  频点1功率: {powers[0]:.2f} W")
        self.log_message(f"  频点2功率: {powers[1]:.2f} W")
        self.log_message(f"  频点3功率: {powers[2]:.2f} W")
        self.log_message(f"  最佳总功率: {result['best_total_power']:.2f} W")
        self.log_message(f"📊 优化p_SWS: {result['optimized_p_SWS'].tolist()}")
        self.log_message("💾 结果已准备就绪,可以保存")
        self.log_area.append("=" * 60)
        
        self.save_button.setEnabled(True)
        self.status_bar.showMessage(f"✓ 优化完成! 总功率: {result['best_total_power']:.2f} W | {self.cpu_info}")
    ...
    def save_results(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, 
            "保存优化结果", 
            f"TWT_3Freq_优化结果_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt", 
            "文本文件 (*.txt);;所有文件 (*)"
        )
        ...
        try:
            with open(path, 'w', encoding='utf-8') as f:
                ...
                f.write("各频点功率:\n")
                f.write("-" * 70 + "\n")
                f.write(f"频点1功率: {self.optimization_result.get('freq1_power', 0):.4f} W\n")
                f.write(f"频点2功率: {self.optimization_result.get('freq2_power', 0):.4f} W\n")
                f.write(f"频点3功率: {self.optimization_result.get('freq3_power', 0):.4f} W\n")
                f.write(f"总功率:    {self.optimization_result['best_total_power']:.4f} W\n\n")
