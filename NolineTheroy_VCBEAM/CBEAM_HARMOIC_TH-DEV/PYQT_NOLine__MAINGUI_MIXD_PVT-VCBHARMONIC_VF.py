...
# 假设计算函数在以下模块中
from Noline_GAIN_MAINCALL_VCBMULTIPLIER_CORE_MIXED_WITH_PVT import (
...
TABLE_COLUMNS_BEFORE = [
    "倍频前Kc (Ω)",
    "倍频前Loss_perunit",
    "倍频前Freq (GHz)",
    "倍频前Vpc (c)",
]
TABLE_COLUMNS_HARMONIC = [
    "倍频段Kc (Ω)",
    "倍频段Loss_perunit",
    "倍频段Freq (GHz)",
    "倍频段Vpc (c)",
...
PARAM_CONFIG = [
    ...
    (
        "p_sws",
        "周期长度 p_SWS (mm)",
        "0.50, 0.50, 0.25, 0.245, 0.240",
        "倍频前和倍频段的周期长度",
    ),
    ("n_unit", "周期数 N_Unit", "20, 6, 28, 28, 28", "各段的周期数"),
    ("w", "束流宽度 w (mm)", "0.2", "电子束水平宽度"),
    ("t", "束流厚度 t (mm)", "0.2", "电子束垂直厚度"),
    ("fn_k", "Fn_K", "2, 1.2", "倍频前和倍频段的填充因子"),
    ("p_in", "输入功率 P_in (W)", "0.005", "输入信号功率"),
    ("loss_attu", "衰减量 (dB)", "30", "衰减段衰减量"),
    ("section_seg_idx", "衰减段索引", "1", "衰减段索引（0开始）"),
    ("harmonic_times", "倍频前后谐波次数数组", "2,1", "倍频前后谐波次数数组,默认[2,1]"),
    ("harmonic_start_idx", "倍频段起始索引", "2", "倍频段开始的段索引（从0开始）"),
    ("vpc_adjust_coef", "Vpc调整系数", "0.80", "Vpc随周期变化的调整系数"),
    ("kc_adjust_coef", "Kc调整系数", "2.0", "Kc随周期变化的调整系数"),
]
...
def calculate_twt_point(params):
    ...
    try:
        ...
        param_str = (
            ...
            f"  输入端倍频谐波次数: {fixed['harmonic_times']}\n"
        ...
        result = calculate_SEGMENT_TWT_NOLINE(
            ...
            Loss_perunit=[
                float(before["Loss_perunit"]),
                float(harmonic["Loss_perunit"]),
            ],  # 倍频前和倍频段的损耗
            ...
            harmonic_times=fixed["harmonic_times"],
...
class TWTCalculator(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.add_row_before_btn.clicked.connect(
            lambda: self.add_table_row(self.table_before)
        )
        self.del_row_before_btn = QPushButton("删除行")
        self.del_row_before_btn.clicked.connect(
            lambda: self.delete_table_row(self.table_before)
        )
        self.clear_table_before_btn = QPushButton("清空表格")
        self.clear_table_before_btn.clicked.connect(
            lambda: self.clear_table(self.table_before)
        )
        ...
        self.add_row_harmonic_btn.clicked.connect(
            lambda: self.add_table_row(self.table_harmonic)
        )
        self.del_row_harmonic_btn = QPushButton("删除行")
        self.del_row_harmonic_btn.clicked.connect(
            lambda: self.delete_table_row(self.table_harmonic)
        )
        self.clear_table_harmonic_btn = QPushButton("清空表格")
        self.clear_table_harmonic_btn.clicked.connect(
            lambda: self.clear_table(self.table_harmonic)
        )
        ...
        self.import_before_btn.clicked.connect(
            lambda: self.import_csv_template("before")
        )
        self.import_harmonic_btn = QPushButton("导入倍频段CSV")
        self.import_harmonic_btn.clicked.connect(
            lambda: self.import_csv_template("harmonic")
        )
    ...
    def add_table_row(self, table):
        ...
        if table == self.table_before:
            default_values = ["1.0", "0", "440", "0.292"]
        else:  # table_harmonic
            default_values = ["2.0", "0.10", "440", "0.289"]
    ...
    def import_csv_template(self, table_type):
        ...
        try:
            with open(path, "r", encoding="utf-8") as f:
                ...
                self.log_message(
                    f"已从 {path} 导入 {rows_added} 行{('倍频前' if table_type == 'before' else '倍频段')}数据"
                )
            ...
            self.log_message(
                f"导入{('倍频前' if table_type == 'before' else '倍频段')}CSV文件时出错: {str(e)}"
            )
    ...
    def parse_params(self):
        ...
        for pid, widget in self.param_widgets.items():
            ...
            # 处理列表参数['p_sws', 'n_unit', 'fn_k', 'section_seg_idx','harmonic_times']
            if pid in ["p_sws", "n_unit", "fn_k", "section_seg_idx", "harmonic_times"]:
                try:
                    # 分割字符串为列表并转换为数值类型
                    ...
                    try:
                        # 对于n_unit，尝试转换为整数
                        if pid == "n_unit" or pid == "harmonic_times" or pid =="section_seg_idx" :
        ...
        if not fixed_params.get("fn_k") or len(fixed_params["fn_k"]) != 2:
            QMessageBox.warning(self, "参数错误", "Fn_K 需要2个值（倍频前和倍频段）")
            return None, None

        if (
            not fixed_params.get("harmonic_times")
            or len(fixed_params["harmonic_times"]) != 2
        ):
            QMessageBox.warning(
                self, "参数错误", "harmonic_times 需要2个值（倍频前和倍频段）"
            )
            return None, None
        ...
        if "harmonic_start_idx" in fixed_params:
            try:
                fixed_params["harmonic_start_idx"] = int(
                    fixed_params["harmonic_start_idx"]
                )
            except:
            ...
            if self.results and self.completed < total_tasks:
                try:
                    ...
                    if "error" in result:
                        self.error_count += 1
                        self.log_message(
                            f"频率 {result['freq']} GHz: {result['error']}"
                        )
                    elif "power" in result:
                        self.success_count += 1
                        self.history.append((result["freq"], result["power"]))
                        self.log_message(
                            f"频率 {result['freq']} GHz: 输出功率 {result['power']:.2f} W"
                        )
                    ...
                    if elapsed > 1:  # 至少3秒后计算剩余时间
                        remaining = (elapsed / max(self.completed, 1)) * (
                            total_tasks - self.completed
                        )
                    ...
                    self.status_label.setText(
                        f"进度: {self.completed}/{total_tasks} {time_msg}"
                    )
    ...
    def finish_calculation(self):
        ...
        try:
            ...
            elapsed = time.time() - self.start_time
            self.log_message(
                f"计算完成! 成功: {self.success_count}, 失败: {self.error_count}"
            )
            ...
            if self.success_count > 0:
                ...
                self.history_results.append(
                    (freqs, powers, f"参数组{self.plot_counter}")
                )
                self.plot_counter += 1
    ...
    def export_data(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "导出计算结果", default_name, "CSV文件 (*.csv)"
        )
