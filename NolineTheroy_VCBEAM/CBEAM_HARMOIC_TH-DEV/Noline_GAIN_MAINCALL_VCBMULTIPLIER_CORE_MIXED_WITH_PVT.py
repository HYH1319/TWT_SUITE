...
from _TWT_CORE_NOLINE_COMPLEX_VCBEAM_HARMONIC_VF import (
...
def calculate_SEGMENT_TWT_NOLINE(
    ...
    def build_segment_input(common_params, seg,f0_GHz_cuurent):
        """构建单个段的计算输入参数"""
        return [
            common_params["I"],
            common_params["V"],
    ...
    def initialize_harmonicfirst_segment(params, common, results, input_power):
        """初始化并计算第一段"""
    ...
    for seg_idx, segment in enumerate(segments):
        # 4.1 准备当前段输入参数
        # 谐波矫正
        f0_GHz_cuurent=common_params["f0_GHz"] / segment["harmonic_times"]
        input_params = build_segment_input(common_params, segment,f0_GHz_cuurent)
        ...
        solver_params = {
            ...
            / (2 * np.pi * (f0_GHz_cuurent * 1e9)),
            "beta_space": calc_result["beta_Space"],
            "y_end": 2 * np.pi * C * (calc_result["互作用长度N"])
            + (results[-1]["y"][-1] if seg_idx > 0 else 0),
        }
        print(f"段 {seg_idx+1} 求解参数: {solver_params}")

        # 4.4 根据段类型处理
        if seg_idx == 0:  # 第一段
            initialize_harmonicfirst_segment(
...
if __name__ == "__main__":
    ...
    input_params = [
        ...
        0.005,  # 输入功率 [W],公共参数
        30,  # 衰减器损耗 [dB],公共参数
        [2, 1],  # 输入端谐波次数harmonic_times，倍频前和倍频段两组值
    ]
