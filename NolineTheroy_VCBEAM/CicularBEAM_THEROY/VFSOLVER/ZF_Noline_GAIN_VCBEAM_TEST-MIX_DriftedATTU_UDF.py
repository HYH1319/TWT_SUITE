...
from _TWT_CORE_NOLINE_COMPLEX_VSUPER_MIX_VF import solveTWTNOLINE_OUTPUT, solveTWTNOLINE_INIT,solveTWTNOLINE_Drift

def main():
    # ========================= 多段参数配置 =========================
    SEGMENTS = [
        {"len": 100, "Vpc": 0.19025644578591866, "p_SWS": 0.297, "Kc": 1.8492148485796647, "Loss_perunit": 0.05, "type": "initial"},
        # {"len": 6, "Vpc": 0.27168, "p_SWS": 0.60, "Kc": 2.0431, "Loss_perunit": 0.07, "type": "attenuator"},
        # {"len": 30, "Vpc": 0.27168, "p_SWS": 0.60, "Kc": 2.0431, "Loss_perunit": 0.07, "type": "O"},
        # {"len": 6, "Vpc": 0.27168, "p_SWS": 0.60, "Kc": 2.0431, "Loss_perunit": 0.07, "type": "attenuator"},
        # {"len": 46, "Vpc": 0.27168, "p_SWS": 0.60, "Kc": 2.0431, "Loss_perunit": 0.07, "type": "O"},
    ]
    Loss_attu = 25
    P_in = 0.05 #输入功率Pin
    # ========================= 全局参数 =========================
    COMMON_PARAMS = {
        "I": 0.08, "V": 9750,"Fn_K": 1.2, 
        "w": 0.16, "t": 0.16,"f0_GHz": 211, 
    }
    ...
    # ========================= 结果处理与可视化 =========================
    process_and_visualize(results, C_list, COMMON_PARAMS, SEGMENTS,beta_e)
...
def build_input_params(common_params, seg):
    ...
    return [
        ...
        common_params["Fn_K"], common_params["f0_GHz"], seg["Vpc"]
    ]
...
def process_and_visualize(results, C_list, common_params, segments,beta_e):
    """结果处理与可视化"""
    # 数据合成
    ...
    Lenth = Y_Finall / (beta_e * np.mean(C_list))*1e3
...
def plot_results(Y, A, theta, u, Lenth, P_Out, final_seg):
    ...
    plt.xlabel("Position Z(mm)", fontsize=10)
